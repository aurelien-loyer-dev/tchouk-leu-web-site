...
const COLS_CLASS: Record<ColsToken, string> = {
  2: "grid-cols-2",
...
function Tile({
  ...
  if (reduce) {
    return (
      <figure ref={ref} className="relative z-10 m-0 cursor-pointer" onClick={handleClick}>
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{
          aspectRatio, borderRadius: rounded, filter,
          x: tx, y: ty, z: tz, rotate: rot, rotateX: rx, skewX: sk,
        }}
        whileHover={handleClick ? { scale: 1.03 } : undefined}
...
export function ScrollTiltedGrid({
  ...
  const gridClass = [
    "mx-auto mt-[10vh] mb-[10vh] grid w-full px-6 py-[15vh]",
      ...
      <div className={gridClass}>
        {items.map((src, i) => (
          <Tile
            ...
            side={i % 2 === 0 ? "L" : "R"}
            config={config}
            onClick={onImageClick ? () => onImageClick(i % images.length, src) : undefined}
          />
        ))}
      </div>
      {loop && <div ref={sentinelRef} aria-hidden className="h-px w-full" />}
    </section>
  );
}
